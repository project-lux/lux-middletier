...
import {
  parseUrlQueryString,
  extractDataParamsFromUrl,
  isValidSearchUrl,
} from "../../../../utils.js";

export class BenchmarkQueriesTestDataProvider extends TestDataProvider {
  /**
   * Constructor
    ...
    try {
      ...
      if (!validation.isValid) {
        ...
        if (this.options.strictValidation) {
          throw new Error(
            `TSV validation failed: ${validation.errors.join(", ")}`
          );
        }
      }
      ...
      console.error(
        `Error reading TSV file ${this.sourcePath}:`,
        error.message
      );
      throw error;
    }
  ...
  parseTsvFile() {
    return new Promise((resolve, reject) => {
      ...
      const stream = fs.createReadStream(this.sourcePath).pipe(
        csv({
      ...
      stream
        .on("data", (data) => {
          ...
          if (!headers) {
            ...
            if (hasExpectedColumns) {
              ...
              if (rowIndex <= 3) {
                ...
                console.log(
                  `Using fallback headers from row ${rowIndex}:`,
                  headers
                );
                return;
              }
          ...
          if (headers) {
            ...
            const hasData = Object.values(cleanData).some(
              (val) => val && val.trim()
            );
            if (hasData) {
        ...
        .on("end", () => {
          ...
          console.log(
            `Parsed ${results.length} data rows from TSV (headers in row ${this.headerRowIndex})`
          );
          resolve(results);
        })
        .on("error", (error) => {
  ...
  convertToJsonQuery(query) {
    ...
    if (andParts.length > 1) {
      ...
      const convertedParts = andParts.map((part) =>
        this.convertSingleFieldQuery(part.trim())
      );
  ...
  getSearchUrlFromRecord(record) {
    ...
    if (keys.length >= 10 && keys[9]) {
      // 9th index = 10th column
  ...
  getColumnValue(record, columnName) {
    ...
    const keys = Object.keys(record);
    if (columnName === "Query" && keys.length >= 5 && keys[4]) {
      // Column E (5th column, index 4)
      return record[keys[4]]?.trim() || "";
    } else if (columnName === "DEV URL" && keys.length >= 8 && keys[7]) {
      // Column H (8th column, index 7)
      return record[keys[7]]?.trim() || "";
    } else if (columnName === "TEST URL" && keys.length >= 9 && keys[8]) {
      // Column I (9th column, index 8)
    ...
    if (columnName.startsWith("param:")) {
      ...
      const matchingParamKey = Object.keys(record).find(
