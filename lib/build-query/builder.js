...
const agentsRelatedToEvents = require('./queries/agentsRelatedToEvents')
...
const conceptsRelatedToEvent = require('./queries/conceptsRelatedToEvent')
...
const eventsWithSets = require('./queries/eventsWithSets')
...
const itemsInSetWithImages = require('./queries/itemsInSetWithImages')
...
const itemsProducedEncounteredInfluencedByAgent = require('./queries/itemsProducedEncounteredInfluencedByAgent')
const itemsSubjectOfWork = require('./queries/itemsSubjectOfWork')
...
const placesRelatedToEvent = require('./queries/placesRelatedToEvent')
...
const worksCreatedPublishedInfluencedByAgent = require('./queries/worksCreatedPublishedInfluencedByAgent')
...
const worksSubjectOfWork = require('./queries/worksSubjectOfWork')
...
const buildAgentEstimatesQuery = (doc) => ({
  ...
  'lux:agentCreatedPublishedInfluencedWork':
    worksCreatedPublishedInfluencedByAgent(doc.id),
  ...
  'lux:agentMadeDiscoveredInfluencedItem':
    itemsProducedEncounteredInfluencedByAgent(doc.id),
  ...
  'lux:agentWorkCreatedTime': worksCreatedPublishedInfluencedByAgent(doc.id),
  'lux:agentWorkPublishedTime': worksCreatedPublishedInfluencedByAgent(doc.id),
...
const buildEventEstimatesQuery = (doc) => ({
  ...
  'lux:eventRelatedAgents': agentsRelatedToEvents(doc.id),
  'lux:eventRelatedConcepts': conceptsRelatedToEvent(doc.id),
  'lux:eventRelatedPlaces': placesRelatedToEvent(doc.id),
...
const buildWorkEstimatesQuery = (doc) => ({
  ...
  'lux:workSubjectOfItem': itemsSubjectOfWork(doc.id),
  'lux:workSubjectOfWorks': worksSubjectOfWork(doc.id),
})
...
const buildSetEstimatesQuery = (doc) => {
  const setQuery = {
    ...
    'lux:setEvents': eventsWithSets(doc.id),
    ...
    'lux:setItemsWithImages': itemsInSetWithImages(doc.id),
  }
...
const prepareQuery = (query) => encodeURIComponent(JSON.stringify(deleteScope(query)))
...
const queryBuilders = {
  ...
  'lux:agentCreatedPublishedInfluencedWork': (id) => {
    const q = prepareQuery(worksCreatedPublishedInfluencedByAgent(id))
  ...
  'lux:agentMadeDiscoveredInfluencedItem': (id) => {
    const q = prepareQuery(itemsProducedEncounteredInfluencedByAgent(id))
  ...
  'lux:setEvents': (id) => {
    const q = prepareQuery(eventsWithSets(id))
    return `${config.searchUriHost}/api/search/event?q=${q}`
  },
  ...
  'lux:setItemsWithImages': (id) => {
    const q = prepareQuery(itemsInSetWithImages(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  'lux:typeForAgent': (id) => {
  ...
  'lux:workSubjectOfItem': (id) => {
    const q = prepareQuery(itemsSubjectOfWork(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  'lux:workSubjectOfWork': (id) => {
    const q = prepareQuery(worksSubjectOfWork(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
}
