...
const keyFuncNameMap = {
  ...
  event: {
    ...
    'lux:eventObjectTypesAbout': queries.itemsInfluencedByEvent.name,
    ...
    'lux:eventWorksAbout': queries.worksAboutConceptsInfluencedByEvent.name,
    'lux:eventWorkTypesUsed': queries.worksForEvent.name,
    'lux:eventWorkTypesAbout': queries.worksAboutConceptsInfluencedByEvent.name,
  },
  item: {
  ...
  set: {
    ...
    'lux:setIncludedWorks': queries.worksInSet.name,
    ...
    'lux:setUnit': queries.workById.name,
    'lux:setItemsWithImages': queries.itemsInSetWithImages.name,
    ...workKeyFuncNameMap,
  },
  work: workKeyFuncNameMap,
}

const buildEstimatesQuery = (doc) => {
  if (types.isAgent(doc.type)) {
    return helper.buildCombinedQuery(doc.id, keyFuncNameMap[['agent']])
  }
  if (types.isConcept(doc.type)) {
    return helper.buildCombinedQuery(doc.id, keyFuncNameMap[['concept']])
  }
  if (types.isEvent(doc.type)) {
    return helper.buildCombinedQuery(doc.id, keyFuncNameMap[['event']])
  }
  if (types.isItem(doc.type)) {
    return helper.buildCombinedQuery(doc.id, keyFuncNameMap[['item']])
  }
  if (types.isPlace(doc.type)) {
    return helper.buildCombinedQuery(doc.id, keyFuncNameMap[['place']])
  }
  if (types.isSet(doc.type)) {
    return helper.buildCombinedQuery(doc.id, keyFuncNameMap[['set']])
  }
  if (types.isWork(doc.type)) {
    return helper.buildCombinedQuery(doc.id, keyFuncNameMap[['work']])
  }
  return null
}
...
const queryBuilders = {
  ...
  'lux:agentRelatedSubjects': (id) => {
    const q = helper.prepareQuery(queries.worksCreatedByAgent(id))
  ...
  'lux:agentWorkAbout': (id) => {
    const q = helper.prepareQuery(queries.worksAboutAgent(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
  ...
  'lux:conceptChildren': (id) => {
    const q = helper.prepareQuery(queries.childrenOfConcept(id))
    return `${config.searchUriHost}/api/search/concept?q=${q}`
  },
  'lux:conceptInfluencedConcepts': (id) => {
    const q = helper.prepareQuery(queries.conceptsInfluencedByConcept(id))
    return `${config.searchUriHost}/api/search/concept?q=${q}`
  },
  'lux:conceptItemEncounteredTime': (id) => {
    const q = helper.prepareQuery(queries.itemsOfTypeOrMaterial(id))
    return `${config.searchUriHost}/api/facets/item?q=${q}&name=itemEncounteredDate`
  },
  ...
  'lux:conceptRelatedWorks': (id) => {
    const q = helper.prepareQuery(queries.worksRelatedToConcept(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
  ...
  'lux:departmentItems': (id) => {
    const q = helper.prepareQuery(queries.itemsForDepartment(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  'lux:eventConceptsInfluencedBy': (id) => {
    const q = helper.prepareQuery(queries.conceptsSubjectsForPeriod(id))
    return `${config.searchUriHost}/api/search/concept?q=${q}`
  },
  'lux:eventIncludedItems': (id) => {
    const q = helper.prepareQuery(queries.itemsForEvent(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  ...
  'lux:eventObjectTypesAbout': (id) => {
    const q = helper.prepareQuery(queries.itemsInfluencedByEvent(id))
    return `${config.searchUriHost}/api/facets/item?q=${q}&name=itemTypeId`
  },
  ...
  'lux:eventWorksAbout': (id) => {
    const q = helper.prepareQuery(queries.worksAboutConceptsInfluencedByEvents(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
  'lux:eventWorkTypesUsed': (id) => {
    const q = helper.prepareQuery(queries.worksForEvent(id))
    return `${config.searchUriHost}/api/facets/work?q=${q}&name=workTypeId`
  },
  ...
  'lux:genderForAgent': (id) => {
    const q = helper.prepareQuery(queries.agentsWithGender(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:itemArchive': (id) => {
    const q = helper.prepareQuery(queries.archivesWithItem(id))
    return `${config.searchUriHost}/api/search/set?q=${q}`
  },
  'lux:itemEvents': (id) => {
    const q = helper.prepareQuery(queries.eventsWithItem(id))
    return `${config.searchUriHost}/api/search/event?q=${q}&sort=eventStartDate:asc`
  },
  'lux:itemDepartment': (id) => {
    const q = helper.prepareQuery(queries.departmentsWithItem(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:itemUnit': (id) => {
    const q = helper.prepareQuery(queries.itemById(id))
    return `${config.searchUriHost}/api/facets/item?q=${q}&name=responsibleUnits`
  },
  'lux:itemWorksAbout': (id) => {
    const q = helper.prepareQuery(queries.worksAboutItem(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
  'lux:nationalityForAgent': (id) => {
    const q = helper.prepareQuery(queries.agentsWithNationality(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:occupationForAgent': (id) => {
    const q = helper.prepareQuery(queries.agentsWithOccupation(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:placeActiveAgent': (id) => {
    const q = helper.prepareQuery(queries.agentsActiveAtPlace(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:placeBornAgent': (id) => {
    const q = helper.prepareQuery(queries.agentsBornAtPlace(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:placeCreatedWork': (id) => {
    const q = helper.prepareQuery(queries.worksCreatedAtPlace(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
  'lux:placeDepictedAgentsFromRelatedWorks': (id) => {
    const q = helper.prepareQuery(queries.worksRelatedToPlace(id))
  ...
  'lux:placeDiedAgent': (id) => {
    const q = helper.prepareQuery(queries.agentsDiedAtPlace(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:placeEvents': (id) => {
    const q = helper.prepareQuery(queries.eventsHappenedAtPlace(id))
    return `${config.searchUriHost}/api/search/event?q=${q}`
  },
  'lux:placeInfluencedConcepts': (id) => {
    const q = helper.prepareQuery(queries.conceptsInfluencedByPlace(id))
    return `${config.searchUriHost}/api/search/concept?q=${q}`
  },
  'lux:placeItemTypes': (id) => {
    const q = helper.prepareQuery(queries.itemsProducedAtPlace(id))
    return `${config.searchUriHost}/api/facets/item?q=${q}&name=itemTypeId`
  },
  'lux:placeMadeDiscoveredItem': (id) => {
    const q = helper.prepareQuery(queries.itemsProducedEncounteredAtPlace(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  'lux:placeParts': (id) => {
    const q = helper.prepareQuery(queries.partsOfPlace(id))
    return `${config.searchUriHost}/api/search/place?q=${q}`
  },
  'lux:placePublishedWork': (id) => {
    const q = helper.prepareQuery(queries.worksPublishedAtPlace(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
  ...
  'lux:placeWorkAbout': (id) => {
    const q = helper.prepareQuery(queries.worksAboutPlace(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
  ...
  'lux:setDepartment': (id) => {
    const q = helper.prepareQuery(queries.departmentsCuratedSet(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:setEvents': (id) => {
    const q = helper.prepareQuery(queries.eventsWithSet(id))
    return `${config.searchUriHost}/api/search/event?q=${q}`
  },
  'lux:setIncludedItems': (id) => {
    const q = helper.prepareQuery(queries.itemsInSet(id))
    return `${config.searchUriHost}/api/search/item?q=${q}&sort=itemArchiveSortId:asc`
  },
  'lux:setIncludedWorks': (id) => {
    const q = helper.prepareQuery(queries.worksInSet(id))
    return `${config.searchUriHost}/api/search/work?q=${q}&sort=workArchiveSortId:asc`
  },
  ...
  'lux:setUnit': (id) => {
    const q = helper.prepareQuery(queries.workById(id))
    return `${config.searchUriHost}/api/facets/work?q=${q}&name=responsibleUnits`
  },
  'lux:setItemsWithImages': (id) => {
    const q = helper.prepareQuery(queries.itemsInSetWithImages(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  'lux:typeForAgent': (id) => {
    const q = helper.prepareQuery(queries.agentsClassifiedAs(id))
    return `${config.searchUriHost}/api/search/agent?q=${q}`
  },
  'lux:typeForEvent': (id) => {
    const q = helper.prepareQuery(queries.eventsClassifiedAs(id))
    return `${config.searchUriHost}/api/search/event?q=${q}`
  },
  'lux:typeForPlace': (id) => {
    const q = helper.prepareQuery(queries.placesClassifiedAs(id))
    return `${config.searchUriHost}/api/search/place?q=${q}`
  },
  'lux:workCarriedBy': (id) => {
    const q = helper.prepareQuery(queries.itemsCarryingWork(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  'lux:workWorksAbout': (id) => {
    const q = helper.prepareQuery(queries.worksAboutWork(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
}
