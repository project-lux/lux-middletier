...
const keyFuncNameMap = {
  ...
  set: {
    ...
    'lux:setIncludedSets': queries.setsInSet.name,
...
const queryBuilders = {
  ...
  'lux:setIncludedSets': (id) => {
    const q = prepareQuery(queries.setsInSet(id))
    return `${config.searchUriHost}/api/search/set?q=${q}&sort=setArchiveSortId:asc`
  },
