...
const queryBuilders = {
  ...
  'lux:eventObjectTypesAbout': (id) => {
    const q = prepareQuery(queries.itemsInfluencedByEvent(id))
    return `${config.searchUriHost}/api/facets/item?q=${q}&name=itemTypeId`
  },
  ...
  'lux:eventWorksAbout': (id) => {
    const q = prepareQuery(queries.worksAboutConceptsInfluencedByEvent(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
  ...
  'lux:setIncludedWorks': (id) => {
    const q = prepareQuery(queries.worksInSet(id))
  ...
  'lux:setUnit': (id) => {
    const q = prepareQuery(queries.workById(id))
