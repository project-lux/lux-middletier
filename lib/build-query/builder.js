...
const itemsSubjectOfItem = require('./queries/itemsSubjectOfItem')
...
const buildItemEstimatesQuery = (doc) => ({
  ...
  'lux:itemSubjectOf': itemsSubjectOfItem(doc.id),
})
...
const buildWorkEstimatesQuery = (doc) => ({
  ...
  'lux:workSubjectOf': worksSubjectOfWork(doc.id),
})
...
const queryBuilders = {
  ...
  'lux:itemSubjectOf': (id) => {
    const q = prepareQuery(itemsSubjectOfItem(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  'lux:itemUnit': (id) => {
