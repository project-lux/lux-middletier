...
import { buildCombinedQuery, prepareQuery } from "./helper.js";
...
const keyFuncNameMap = {
  agent: {
    ...
    "lux:agentEventsUsingProducedObjects":
      queries.eventsUsingAgentsProducedObjects.name,
...
const queryBuilders = {
  ...
  "lux:agentMadeDiscoveredInfluencedItem": (id) => {
    const q = prepareQuery(
      queries.itemsProducedEncounteredInfluencedByAgent(id)
    );
  ...
  "lux:agentSetInfluencedTime": (id) => {
    const q = prepareQuery(queries.setCreatedPublishedInfluencedByAgent(id));
    return `${config.searchUriHost}/api/facets/set?q=${q}&name=setInfluencedDate`;
  },
  ...
  "lux:itemCurrentHierarchyPage": (id) => {
    ...
    return `${
      config.searchUriHost
    }/api/search/multi?q=${q}&sort=archiveSortId:asc&pageWith=${encodeURIComponent(
      id
    )}`;
  },
  "lux:itemEvents": (id) => {
...
export { buildEstimatesQuery, keyFuncNameMap, queryBuilders };
