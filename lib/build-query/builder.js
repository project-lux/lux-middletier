...
const conceptsClassifiedAs = require('./queries/conceptsClassifiedAs')
...
const itemsClassifiedAs = require('./queries/itemsClassifiedAs')
...
const worksClassifiedAs = require('./queries/worksClassifiedAs')
...
const buildConceptEstimatesQuery = (doc) => ({
  ...
  'lux:typeForConcept': conceptsClassifiedAs(doc.id),
  'lux:typeForEvent': eventsClassifiedAs(doc.id),
  'lux:typeForItem': itemsClassifiedAs(doc.id),
  'lux:typeForPlace': placesClassifiedAs(doc.id),
  'lux:typeForWork': worksClassifiedAs(doc.id),
})
...
const queryBuilders = {
  ...
  'lux:typeForConcept': (id) => {
    const q = prepareQuery(conceptsClassifiedAs(id))
    return `${config.searchUriHost}/api/search/concept?q=${q}`
  },
  'lux:typeForEvent': (id) => {
  ...
  'lux:typeForItem': (id) => {
    const q = prepareQuery(itemsClassifiedAs(id))
    return `${config.searchUriHost}/api/search/item?q=${q}`
  },
  'lux:typeForPlace': (id) => {
  ...
  'lux:typeForWork': (id) => {
    const q = prepareQuery(worksClassifiedAs(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
