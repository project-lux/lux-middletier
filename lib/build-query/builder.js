...
const worksWithEvent = require('./queries/worksWithEvent') // Only for research dataset
...
const buildEventEstimatesQuery = (doc) => ({
  ...
  'lux:eventWorksCausedBy': worksWithEvent(doc.id),
...
const queryBuilders = {
  ...
  'lux:eventWorksCausedBy': (id) => {
    const q = prepareQuery(worksWithEvent(id))
    return `${config.searchUriHost}/api/search/work?q=${q}`
  },
