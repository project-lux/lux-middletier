...
function nanoSecToString(nanoSec) {
  let s = BigInt(nanoSec) / 1_000_000_000n
...
function belongsToAltRoute(req) {
  // Strip query string; electing not to use req.path given rest of code base.
  const path = req.url.split('?')[0] || '/'

  // Split the path and remove falsy segments
  const parts = path.split('/').filter(Boolean)

  // Determine index: 0 for "data", 1 for everything else
  const i = parts[0] === 'data' ? 0 : 1

  const key = parts[i]
  if (!key) return false

  return env.altRouteKeyMap[key] === true
}
