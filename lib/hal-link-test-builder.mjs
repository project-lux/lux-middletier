...
const exampleDocs = {
  ...
  // Combine Works and Sets, since the estimates builder for Sets also generates the links for Works
  worksets: [
...
for (const [scope, docArray] of Object.entries(exampleDocs)) {
  for (const uriSuffix of docArray) {
    ...
    // If we succesfully grabbed the document, add its hal links to the list of unsccessful links.
