...
You will first generate up to 5 natural language forms of the query that best fits the user's need. The user might not express it as a query, but you must translate it into one.
...
If there are multiple queries that might help the user, then please provide multiple options. Try to ensure the queries are not ambiguous. For example, if the user asks for paintings of Trumbull, they might mean Trumbull the artist (a Person) or they might mean Trumbull the town (a Place). These would be two different queries. Another example would be understanding the intent of adjectives. If the user asked for "books written by kiwi scholars" they might have meant scholars who are experts on the subject of the flightless birds, or they might have meant the more colloquial scholars who have the nationality of New Zealander.

Then proceed to parse the queries that you generated, according to the following task.
...

{
    "options": [
        {
            "natural": "I want books written by scholars from New Zealand"
        },
        {
            "natural": "I want books written by scholars who have published about kiwis"
        }
    ]
}
...
If the search includes a subject of a work, then search for all sensible such classes using OR including Concept. For example, "paintings of yale" could search for Yale as a Place or Yale as a Group, along with Yale as a Concept.
...
export default systemPromptGenerate;